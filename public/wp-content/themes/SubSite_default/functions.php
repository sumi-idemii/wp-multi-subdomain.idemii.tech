...
/**
 * SubSite_default child theme functions.
 *
...
if ( ! function_exists( 'subsite_default_enqueue_styles' ) ) {
	/**
	 * Enqueue parent and child styles.
	 */
	function subsite_default_enqueue_styles() {
		...
		if ( file_exists( $index_css_path ) ) {
			wp_enqueue_style(
				'subsite-default-index-css',
				get_stylesheet_directory_uri() . '/assets/css/index.css',
				array( 'subsite-default-style' ),
				filemtime( $index_css_path )
			);
		}
	}
}
add_action( 'wp_enqueue_scripts', 'subsite_default_enqueue_styles' );

if ( ! function_exists( 'subsite_default_enqueue_scripts' ) ) {
	/**
	 * Enqueue child theme scripts.
	 */
	function subsite_default_enqueue_scripts() {
		// assets/js/vendor.js を読み込む（依存関係として先に読み込む）
		wp_enqueue_script(
			'subsite-default-vendor-js',
		...
		wp_enqueue_script(
			'subsite-default-main-js',
			get_stylesheet_directory_uri() . '/assets/js/main.js',
			array( 'jquery', 'subsite-default-vendor-js' ),
			wp_get_theme()->get( 'Version' ),
			true // フッターで読み込む
		);
...
add_action( 'wp_enqueue_scripts', 'subsite_default_enqueue_scripts' );

if ( ! function_exists( 'subsite_default_enqueue_admin_styles' ) ) {
	/**
	 * Enqueue admin styles for editor.
	 */
	function subsite_default_enqueue_admin_styles() {
		// assets/css/admin/editor.css を管理画面で読み込む
		wp_enqueue_style(
			'subsite-default-editor-css',
...
add_action( 'enqueue_block_editor_assets', 'subsite_default_enqueue_admin_styles' );