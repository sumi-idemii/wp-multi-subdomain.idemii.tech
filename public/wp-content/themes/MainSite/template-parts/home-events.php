 ...
 * トップページ表示にチェックがONのイベント記事を表示
 *
 * @package wp-multi-subdomain.idemii.tech
 */

// チェックがONのイベント記事を取得（マルチサイト対応）
$events_posts = get_top_page_posts_by_type('events');

// イベント記事が存在する場合のみ表示
if (!empty($events_posts)) :
    // 最大8件に制限
    $events_posts = array_slice($events_posts, 0, 8);
    
    // 記事データを直接使用（WP_Queryを使わない）
    $display_events_posts = $events_posts;
else :
    ...
    $display_events_posts = null; // WP_Queryを使用
endif;

// 表示するイベント記事がある場合
if (!empty($display_events_posts) || (isset($events_query) && $events_query->have_posts())) :
    if (!empty($display_events_posts)) :
        // チェックがONの記事を表示（マルチサイト対応）
        foreach ($display_events_posts as $post_data) :
